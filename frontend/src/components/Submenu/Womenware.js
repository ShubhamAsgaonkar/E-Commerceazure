...
import AddWomenClothes from "./AddWomenClothes";
...
const WomenMenu = () => {
  ...
  const addProduct = () => {
    Axios.get("http://localhost:3002/womenware").then((res) => {
      setProduct(res.data);
    });
  };
  ...
  return (
    <>
      ...
      {user.role === "admin" && <AddWomenClothes />}
          ...
          return (
            <div className="card-d bg-a_primary rounded white" key={i}>
                ...
                <div>
                  <div className="delete-icon">
                    <DeleteIcon
                      onClick={() => {
                        Axios.delete(
                          `http://localhost:3002/womenware/delete/${e.id}`
                        );
                        addProduct();
                      }}
                    />
              ...
              <img
                ...
                alt="Denim Jeans"
                style={{ width: "100%" }}
                onClick={() => history.push(`/descProduct2/${e.id}`)}
              />
